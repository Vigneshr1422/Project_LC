...
// import { useEffect, useState } from "react";
// import { useNavigate } from "react-router-dom";
// import axios from "axios"; // ✅ Only one clean axios import here
// import Loader from "../components/Loader"; 
// import { 
//   Database, 
//   HardDrive, 
...
//   // 🎯 FIX: Explicitly Parse Number to Avoid String Concatenation/Math bugs
//   const calculateEstimatedRemainingUsers = (freeMB) => {
//     const validFreeMB = Number(freeMB || 0);
//     if (validFreeMB <= 0) return 0;
    
//     // Average size of an invoice document with full payload data = ~0.05MB to 0.2MB
//     const avgInvoiceDocSizeMB = 0.2; 
//     return Math.floor(validFreeMB / avgInvoiceDocSizeMB);
//   };
...
//   // 🎯 Math Conversions Secure Fallback Values
//   const totalMB = Number(storage?.totalMB || 512); 
//   const usedMB = Number(storage?.usedMB || 0);
//   const remainingMB = Number(storage?.remainingMB || (totalMB - usedMB));
  
//   // 🎯 Accurate Calculation call passing parsed data parameter
//   const estimatedMoreEntries = calculateEstimatedRemainingUsers(remainingMB);
...
//           }}
//           disabled={refreshing}
//           className="w-full sm:w-auto flex items-center justify-center gap-2 bg-white border border-gray-200 px-4 py-3 rounded-xl font-bold text-sm shadow-sm hover:bg-gray-50 active:scale-95 transition-all text-gray-700 disabled:opacity-60 cursor-pointer"
//         >
...
//         <button
//           onClick={() => navigate(-1)}
//           className="w-full sm:w-auto group flex items-center justify-center gap-2 bg-gradient-to-r from-[#962a27] to-[#7a1e1b] text-white px-8 py-3.5 rounded-xl font-bold shadow-md hover:shadow-xl transition-all cursor-pointer"
//         >
